...
function addPopupForSettlements(feature, layer, map, isAdmin) {
    ...
    if (isAdmin) {
        $(container).children('#adminControl').html("<button type='button' id='delete' class='btn btn-danger'>Delete</button> <button type='button' id='download' class='btn btn-default'>Download</button>");
    }
    layer.bindPopup(container[0]);
